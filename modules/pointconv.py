...
class PointConvDensityEncoder(nn.Module):
    def __init__(self):
        super(PointConvDensityEncoder, self).__init__()
        feature_dim = 0
        self.sa1 = PointConvDensitySetAbstraction(
            npoint=256, nsample=32, in_channel=feature_dim + 3, mlp=[64, 64, 128], bandwidth=0.1, group_all=False)
        self.sa2 = PointConvDensitySetAbstraction(
            npoint=128, nsample=32, in_channel=128 + 3, mlp=[128, 128, 256], bandwidth=0.2, group_all=False)
    ...
    def forward(self, data):
        xyz = data['human_points']
        B, T, N, _ = xyz.shape
        xyz = xyz.reshape(-1, N, 3)  # (B * T, N, 3)
        xyz = xyz.transpose(1, 2)
        l1_xyz, l1_points = self.sa1(xyz, None)
        l2_xyz, l2_points = self.sa2(l1_xyz, l1_points)
        _, l3_points = self.sa3(l2_xyz, l2_points)
        x = l3_points.reshape(B, T, -1)
        return x
